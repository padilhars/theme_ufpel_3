...
define([
    ...
    const CourseHeader = {
         ...
         * 
         * @method setupExpandCollapse
         * @return {void}
         */
        setupExpandCollapse: function() {
            // Check if expand/collapse is enabled
            const isCollapsible = this.element.dataset.collapsible === 'true';
            if (!isCollapsible) {
                return;
            }
            ...
            toggleBtn.className = 'btn btn-sm btn-secondary header-toggle';
            toggleBtn.setAttribute('data-action', 'toggle-header');
            toggleBtn.innerHTML = '<i class="fa fa-chevron-up" aria-hidden="true"></i>';
            
            this.element.querySelector(this.SELECTORS.CONTENT).appendChild(toggleBtn);

            // Get saved state from localStorage
            const savedState = localStorage.getItem('ufpel_course_header_state');
        ...
        refresh: function(courseId) {
            this.element.classList.add(this.CLASSES.LOADING);
            
            return Ajax.call([{
                methodname: 'theme_ufpel_get_course_header_data',
                args: { courseid: courseId }
            }])[0].then(response => {
                return Templates.render('theme_ufpel/course_header_full', response);
            }).then(html => {
                // Replace content
                const temp = document.createElement('div');
                temp.innerHTML = html;
                const newHeader = temp.querySelector(this.SELECTORS.HEADER);
                
                if (newHeader) {
                    this.element.replaceWith(newHeader);
                ...
                this.element.classList.remove(this.CLASSES.LOADING);
            });
        }
    };
